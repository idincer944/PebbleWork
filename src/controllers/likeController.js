...
module.exports = {
  likeEvent: async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Internal Server Error while liking event' });
    }
  },
  ...
  removeLikeEvent: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: 'Event like removed successfully' });
    } catch (error) {
      console.error('Error removing event like:', error);
      return res
        .status(500)
        .json({ error: 'Internal Server Error while removing event like' });
    }
  },
};
